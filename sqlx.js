...
function SQLiteMigrateDB() {
	...
	this.addTableColumns = function(config) {
		Ti.API.info("add table columns migration called for " + config.adapter.collection_name);
		var self = this;

		for (var k in config.columns) {

			var sql = "ALTER TABLE " + config.adapter.collection_name + " ADD COLUMN " + k + " " + self.column(config.columns[k]);
			Ti.API.info(sql);
			db.execute(sql);
		}
	};
...
function Migrate(migrations) {
	...
	var migrationIds = {};
	// cache for latest mid by model name
